...
/**
 * Integration tests for the {@link SubscriptionMemberController} REST controller.
 */
@SpringBootTest
@AutoConfigureMockMvc
class SubscriptionMemberControllerTest {
    ...
    public static SubscriptionMember createEntity(EntityManager em) {
        ...
        if (TestUtil.findAll(em, Plan.class).isEmpty()) {
            plan = PlanControllerTest.createEntity(em);
            em.persist(plan);
            em.flush();
        } else {
    ...
    public static SubscriptionMember createUpdatedEntity(EntityManager em) {
        ...
        Plan plan;
        if (TestUtil.findAll(em, Plan.class).isEmpty()) {
            plan = PlanControllerTest.createUpdatedEntity(em);
            em.persist(plan);
            em.flush();
        } else {
            plan = TestUtil.findAll(em, Plan.class).get(0);

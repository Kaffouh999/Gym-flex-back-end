...
class OrderControllerTest {
    ...
    public static Order createEntity(EntityManager em) {
        ...
        if (TestUtil.findAll(em, Product.class).isEmpty()) {
            product = ProductControllerTest.createEntity(em);
            em.persist(product);
            em.flush();
        } else {
        ...
        if (TestUtil.findAll(em, Cart.class).isEmpty()) {
            cart = CartControllerTest.createEntity(em);
            em.persist(cart);
            em.flush();
        } else {
    ...
    public static Order createUpdatedEntity(EntityManager em) {
        ...
        Product product;
        if (TestUtil.findAll(em, Product.class).isEmpty()) {
            product = ProductControllerTest.createUpdatedEntity(em);
            em.persist(product);
            em.flush();
        } else {
            product = TestUtil.findAll(em, Product.class).get(0);
        ...
        Cart cart;
        if (TestUtil.findAll(em, Cart.class).isEmpty()) {
            cart = CartControllerTest.createUpdatedEntity(em);
            em.persist(cart);
            em.flush();
        } else {
            cart = TestUtil.findAll(em, Cart.class).get(0);

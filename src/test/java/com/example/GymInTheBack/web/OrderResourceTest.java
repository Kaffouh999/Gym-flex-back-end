...
import com.example.GymInTheBack.services.mappers.OrderMapper;
...
class OrderResourceTest {
    ...
    public static Order createEntity(EntityManager em) {
        Order order = new Order().quantity(DEFAULT_QUANTITY);
        // Add required entity
        Product product;
        ...
        if (TestUtil.findAll(em, Cart.class).isEmpty()) {
            cart = CartResourceTest.createEntity(em);
            em.persist(cart);
            em.flush();
        } else {
    ...
    public static Order createUpdatedEntity(EntityManager em) {
        Order order = new Order().quantity(UPDATED_QUANTITY);
        // Add required entity
        Product product;
        if (TestUtil.findAll(em, Product.class).isEmpty()) {
        ...
        Cart cart;
        if (TestUtil.findAll(em, Cart.class).isEmpty()) {
            cart = CartResourceTest.createUpdatedEntity(em);
            em.persist(cart);
            em.flush();
        } else {
            cart = TestUtil.findAll(em, Cart.class).get(0);
    ...
    void putExistingOrder() throws Exception {
        ...
        updatedOrder.quantity(UPDATED_QUANTITY);
